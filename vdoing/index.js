...
module.exports = (options, ctx) => {
  console.log('000----------------',ctx)
  const { siteData } = ctx
  const themeConfig = ctx.options.themeConfig
  const sourceDir = ctx.options.source
  // base路径
  base = siteData.base || ''
  console.log('1111---------------------------1')
  ...
  // 分类页
  if (themeConfig.category && themeConfig.category !== false) {
  ...
  const isAlgoliaSearch = (
    themeConfig.algolia
    || Object
      .keys(siteData.locales && themeConfig.locales || {})
      .some(base => themeConfig.locales[base].algolia)
  )
  console.log('1111---------------------------6',isAlgoliaSearch)
...
function getCardListDOM(dataList, row, config) {
  console.log('2222222222222222',dataList)
